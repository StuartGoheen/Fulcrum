...
(function () {
  ...
  function buildDay1Html(scene, partyCache, campaignState) {
    ...
    html += '<input type="text" class="tt-recap-gm" data-recap-gm placeholder="Your GM handle" maxlength="40" />';
    ...
    html += '</div>';
    var lastRegen = state.day1.lastRecapRegen;
    if (lastRegen && lastRegen.at) {
      var who = esc(lastRegen.gmName || 'GM');
      var when = lastRegen.at;
      try {
        var dt = new Date(lastRegen.at);
        if (!isNaN(dt.getTime())) when = dt.toLocaleString();
      } catch (_) { /* keep raw */ }
      html += '<div class="tt-recap-audit" data-recap-audit>Last regenerated by <strong>' + who + '</strong> at ' + esc(when) + '.</div>';
    }
    html += '</div>';
  ...
  function bindDay1(panel, scene, partyCache, campaignState, socket, refresh) {
    ...
    if (regen) {
      regen.addEventListener('click', function () {
        ...
        if (gmName) payload.gmName = gmName;
  ...
  function _styleBlock() {
    return '<style>' +
      ...
      '.tt-recap-actions{display:flex;align-items:center;gap:.5rem;margin-top:.3rem;flex-wrap:wrap;}' +
      '.tt-recap-gm{background:#15151f;color:#d8d8e8;border:1px solid #2a2a3a;border-radius:3px;padding:.2rem .35rem;font-size:.6rem;font-family:inherit;width:8rem;}' +
      ...
      '.tt-recap-audit{margin-top:.3rem;font-size:.6rem;color:#9ca3af;font-style:italic;}' +
      '.tt-recap-audit strong{color:#f5d56b;font-style:normal;}' +
