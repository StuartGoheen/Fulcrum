...
function _formatTournamentRecapBody(state) {
  ...
  const lastRegen = state && state.day1 && state.day1.lastRecapRegen;
  if (lastRegen && lastRegen.at) {
    const who = (lastRegen.gmName && String(lastRegen.gmName).trim()) || 'GM';
    let when = lastRegen.at;
    try {
      const dt = new Date(lastRegen.at);
      if (!isNaN(dt.getTime())) when = dt.toUTCString();
    } catch (_) { /* keep raw */ }
    lines.push('');
    lines.push('— Last regenerated by ' + who + ' at ' + when + ' —');
  }
...
function registerHandlers(io) {
  io.on('connection', (socket) => {
    ...
    socket.on('tournament:regenerate-day1-recap', async ({ sceneId, gmNotes, gmName } = {}) => {
      if (socket.data.role !== 'gm') {
      ...
      try {
        ...
        // possibly-stale DB state.
        if (typeof gmNotes === 'string' && state.day1.recapNotes !== gmNotes) {
          state.day1.recapNotes = gmNotes;
        }
        // Audit trail: record who regenerated and when, and append to the in-panel log.
        let cleanGm = 'GM';
        if (typeof gmName === 'string') {
          const collapsed = gmName.replace(/[\r\n\t\u0000-\u001F\u007F]+/g, ' ').replace(/\s+/g, ' ').trim();
          if (collapsed) cleanGm = collapsed.slice(0, 40);
        }
        const nowIso = new Date().toISOString();
        state.day1.lastRecapRegen = { gmName: cleanGm, at: nowIso };
        state.day1.log = state.day1.log || [];
        state.day1.log.unshift({ ts: nowIso, text: 'Recap regenerated by ' + cleanGm + '.' });
        ...
        try {
          ...
          console.warn('[socket] tournament:regenerate-day1-recap: failed to persist state:', e && e.message);
        }

        const result = await _saveTournamentRecapToJournal(state, String(sceneId), { regenerate: true });

        // Broadcast the updated tournament state so the Day 1 panel + player strip refresh.
        try {
          const allRows = await pool.query('SELECT key, value FROM campaign_state');
          const fullState = allRows.rows.reduce((acc, row) => {
            try { acc[row.key] = JSON.parse(row.value); }
            catch { acc[row.key] = row.value; }
            return acc;
          }, {});
          io.to('gm').emit('state:sync', { state: fullState });
          const playerSockets = await io.in('players').fetchSockets();
          for (const ps of playerSockets) {
            ps.emit('state:sync', { state: _filterStateForPlayers(fullState, ps.data && ps.data.characterId) });
          }
        } catch (e) {
          console.warn('[socket] tournament:regenerate-day1-recap: state broadcast failed:', e && e.message);
        }

        if (result && result.entryId) {
          io.emit('journal:updated', { entryId: result.entryId });
          if (result.updated) {
            console.log(`[socket] Tournament Day 1 recap regenerated by ${cleanGm} (entry #${result.entryId}).`);
          } else if (result.created) {
            console.log(`[socket] Tournament Day 1 recap created on regenerate by ${cleanGm} (entry #${result.entryId}).`);
          }
          socket.emit('tournament:recap-regenerated', {
