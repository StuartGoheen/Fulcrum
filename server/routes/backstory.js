...
function buildPrompt(payload) {
  ...
  if (personalDestiny) {
    ...
    destinySection += `\nThis character's fate pulls ${pronouns.obj} toward ${personalDestiny.name.toLowerCase()}. Weave this thread into the backstory — not as a stated goal, but as a pattern visible in ${pronouns.pos} choices and circumstances. Show a specific past moment where ${pronouns.sub} acted in the spirit of the hope recovery trigger, AND a specific past moment where ${pronouns.sub} acted in the spirit of the toll recovery trigger. These two moments should feel like a moral fork in the road — the tension between them IS the character.${personalDestiny.coreQuestion ? ` The core question ("${personalDestiny.coreQuestion}") should linger as subtext — do not state it, but let it echo through the events.` : ''}`;
  }
...
- ABSOLUTE BAN ON GAME MECHANICS IN PROSE: Never reference die sizes (D4, D6, D8, D10, D12), arena ratings, discipline names as mechanical labels, dice rolls, exploding dice, rerolls, skill checks, tier numbers, or any RPG terminology in the narrative text. The backstory is pure fiction — the reader should never sense a character sheet behind the words. Show what the character CAN and CANNOT do through action and consequence, never through mechanical description.
...
- The arena profile and discipline ratings are provided for YOUR reference to understand the character's capabilities. NEVER mention arena names, die sizes, or discipline ratings in the prose. Translate them into narrative reality:
  * Low arenas mean the character is visibly limited in that dimension. Weak physique = slight, frail, or unimposing body. Weak reflex = slow, clumsy, poor coordination. Weak grit = brittle willpower, folds under pressure. Weak wits = slow thinker, easily confused in complex situations. Weak presence = forgettable, socially invisible, or off-putting.
  * High arenas are defining strengths that shaped the character's history and reputation. Show these through action and consequence.
- Formative disciplines (the strong ones) are skills honed beyond casual familiarity. Show them being USED in the narrative through specific scenes — someone who vanished into crowds, someone who kept people alive with improvised tools, someone who could strip and reassemble a hyperdrive blindfolded.
- Incompetent disciplines (the weak ones) are critical character gaps. These are things the character CANNOT do and has learned to avoid or work around:
  * Show incompetencies as visible absences or coping strategies. A character who cannot lie survives by avoiding situations that require deception. One who fumbles with machinery distrusts technology. One who freezes in medical emergencies watched someone bleed out because they didn't know what to do.
...
DESTINY GUIDANCE:
- The destiny alignment shapes the character's moral compass and narrative voice. The personal destiny (if present) represents a fate the character is drawn toward — show its pattern in their past choices, not as a stated goal.
- HOPE vs TOLL DUALITY: The hope and toll recovery triggers describe two opposing METHODS the character might use to recover from crisis — one selfless, one destructive. You MUST plant seeds of BOTH methods in the backstory. Show at least one moment where the character chose the hopeful path (or wanted to), AND at least one moment where they chose the darker path (or were tempted to). This tension is the emotional core of the character.
- CORE QUESTION: If a core question is provided, it should haunt the backstory like an unanswered echo. Do not state the question literally — instead, show situations where the answer was ambiguous, where the character's actions left the question hanging. The reader should feel the question without it being spoken.
- ADVANCE TRIGGER: If an advance trigger is provided, show the beginning of this pattern — a first instance where the character's actions moved toward the trigger condition, even if incompletely or accidentally.
