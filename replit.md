...
- `data/kits.json` — Vocation definitions (Kit System v2). 9 vocations: The Gunslinger (ranged/reflex), The Shockboxer (brawl/physique), The Ghost (stealth/reflex), The Ichor Witch (alter_spark/presence), The Noble (charm/presence), The Investigator (investigation/wits), The Seer (sense_spark/wits), The Altus Sopor (control_spark/grit), The Telekinetic Savant (alter_spark/presence). Each has 5-tier ability tree with typed abilities (passive/gambit/maneuver/exploit/permission/action), favoredDiscipline, tags, effect tracks, risk fields, and `imageUrl` (placeholder SVGs in `assets/vocations/`). Kit budget is 5 points (1pt per tier). Tier cap enforced by linked discipline die: D4→T1, D6→T2, D8→T3, D10→T4, D12→T5. Vocations screen uses `buildPhaseCarousel` flat carousel system (same as Species/Phases/Destiny) with `buildVocationCardFlat` card builder. In-place card rebuild on tier changes via `rebuildCurrentVocationCard`; stale-slide sync via `rebuildActiveVocationSlide` on carousel navigation.
