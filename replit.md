...
- `data/starship-modifications.json` — 18 starship modifications (6 Handling, 6 Shields, 6 Engines) gated by hardware die level. D8: 2 mods (1 passive + 1 gambit), D10: 2 mods (1 passive + 1 gambit), D12: 2 exclusive passives (pick one = ship identity). D12 Handling: Corellian Wake (doubles 8+ bonus, unlocks Micro-Jump via Handling) vs this is the only D12 option. D12 Shields: Impenetrable Baffles (+1 Buffered per Reinforce tier) vs Aegis Protocol (extend shields to allies). D12 Engines: Class Zero Hyperdrive (0.5 hyperdrive + Micro-Jump via Engines) vs Oversized Reactor Core (Engines soaks Hull Rating + 2 on impairment). Sensors and Fire Control mods not yet designed.
