...
- `data/threats.json` — NPC Threat Builder data: Fulcrum rules system (Tier 0-5, Arena Scores 1-5, 13 Roles, 4 Classifications, 52 Traits, 26 Tags). 5 threat categories: character (scale 1), vehicle (scale 2), starship (scale 3), capital_ship (scale 4), station (scale 5). Scale traits have `autoApply: true`. Traits support `powerMod` (Turbolaser Battery, Overgunned, Mounted Weapon) and `initiativeMod` (Advanced Sensors, Command Ship). Ship-flavored arena labels: Firepower/Handling/Hull/Sensors/Command for ships; Armor/Handling/Hull/Sensors/Presence for vehicles. Ship details: hullType, crew, speed, shields, hyperdrive, sensors, cargo
