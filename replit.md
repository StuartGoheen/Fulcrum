...
│   ├── combat-tracker.js   # GM Combat Tracker — two-panel layout (initiative rail + detail panel), NPC disposition badges (Enemy/Neutral/Ally) with click-to-cycle in initiative rail synced to player UI, disposition-colored map tokens (red enemy/yellow neutral/green ally), auto-numbered duplicate NPCs (both count>1 expansion and separately-saved duplicates detected post-load), Condition Command Panel in right column (replaces floating palettes), combo condition system (Surprised/Stunned/Pinned stored as single atomic entries with expandCondIds() unpacking components for mechanical calculations), NPC conditions stored as objects {id,duration,arena?} with duration badges, condition panel with target selector/condition list/duration picker/arena picker/apply button, active conditions summary across all combatants, "Push to PC" button on NPC cards, right column tab switching (Conditions/Glossary) during combat, mobile tab integration, WebSocket Join Battle flow (surprised = single lingering combo condition), bidirectional condition sync, tactical map with token movement and persistent GM position overrides (gmStartingPositions saved to scene JSON via PUT /api/campaign/scene/:id/positions), unplaced NPC bar below map with click-to-place flow, combat token overlay layer on tactical map (colored circles per combatant positioned in zones — red=enemy, green=ally, yellow=neutral, blue=PC — with grid layout for multiple tokens in same zone, rendered via TacticalMapViewer.renderCombatTokens() called after every tokenPositions change), two-pass position matching (numbered NPCs match numbered keys first, then fuzzy fallback), PC hot-join support (late player connects mid-combat → new pcSlot created on join-battle-result, token placed at PC default zone, inserted into initiative order at rolled value, synced to server), combat NPC loading uses scene NPCs as source of truth (ignores encounter composition), full server-side state persistence (combat:sync-state), GM refresh reconnect (restoreFromState) with tactical map auto-reopen from broadcastedMapKey, player reconnect auto-restores initiative tracker + tactical map if broadcast active, server tracks _broadcastedMapKey (set on map:broadcast, cleared on map:dismiss/combat:end), 10-second token position heartbeat during active combat for drift correction, combat:ended auto-dismisses player tactical map panel, mid-combat NPC add/remove/edit via Threat Builder integration with auto-numbering on add, persistent GM Combat Log (collapsible panel under tracker recording start/scripted-escalations/manual condition apply/remove (NPC + PC)/vitality deltas/KOs/round transitions, last 500 entries, synced via combat:sync-state, restored across GM refresh, end-of-encounter summary modal with totals/KO list/standing list/full log + Copy Log button)
...
- `data/galaxy-planets.json` — 9 campaign-specific planets (Jakku, Ajan Kloss, Takodana, Endor, Bespin, Malpaz, Eriadu, Batuu, Xala) with normalized galactic coordinates derived from canonical grid squares (A-U columns × 1-21 rows). Each entry has name, x, y, region, sector, gridSquare, desc, and campaign:true flag. Grid orientation: letters A-U = columns left→right, numbers 1-21 = rows top→bottom. Used by the interactive Galaxy Map.
- `data/downtime.json` — Downtime & Interlude system. Defines GM workflow (5 steps: determine window → select system activities → add adventure interludes → choose entanglements → run downtime), three downtime period types (Short Rest/Long Rest/Extended), 7 system-level activities (Rest & Recovery, Tinkering, Research & Analysis, Requisition & Resupply, Training & Drill, Personal Scene, Crew Bonding). Activities with checks use the standard Fulcrum resolution (Fleeting/Masterful/Legendary tiers). Training grants [Drilled] condition (Step Up Control Die, one use). Activities without checks (Training, Personal Scene, Crew Bonding) are narrative. Adventure-specific interludes extend this system with authored content.
...
- `js/lib/persist.js` exposes `window.Persist` (`get/set/remove/clear/migrate` over namespaced `eote.*` localStorage keys) and is the **only allowed entry point for browser storage** in client code. All client `localStorage.*` calls live inside this helper; never call `window.localStorage` directly from `js/*.js` or inline scripts in `public/**/*.html`. Use short keys (e.g. `'theme'`, `'col-widths'`, `'cb.tt-open-panels'`); the `eote.` prefix is added automatically. To rename or relocate an existing key, call `Persist.migrate(oldRawKey, newKey)` once at module init so saved data carries over.
- Hidden-tab work: every recurring timer (`setInterval`) in client JS is visibility-aware — the work skips while `document.hidden` and refreshes once on `visibilitychange`. This is implemented in `js/socket-client.js` (player scene poll), `js/clock-widget.js` (socket-boot retry + clock refresh — used by both player and GM pages), `js/opening-crawl.js` (audio fade-in), and `js/glossary-overlay.js` (protocol-droid cooldown tick). The GM-only scripts (`js/command-bridge.js`, `js/combat-tracker.js`, `js/tactical-map.js`, `js/npc-builder.js`, `js/conversation-overlay-gm.js`, `js/tournament-tracker.js`, `js/effect-manager.js`, `js/tts-narration.js`, `js/galaxy-map.js`, `js/vanishing-place-fortress.js`) contain no recurring intervals — only one-shot `setTimeout` debouncers and toast cleanups — so a hidden GM tab is already idle. Any new recurring timer must follow the same `document.hidden` guard.
- Server I/O policy: socket handlers and Express routes must not call synchronous `fs.readFileSync`/`writeFileSync`/`existsSync` in the request/event path — sync disk I/O blocks the Node event loop and stalls every connected client. Use `fs/promises` (`fsp`) and `await`. For static reference data that's read repeatedly, cache the parsed result at module scope behind a memoized in-flight promise (see `loadShipCombatRefData()` in `server/sockets/handlers.js` for the pattern: cache the resolved data, also cache the in-flight promise so concurrent first-callers share one read, and clear the promise on error so retries can reload).
- Socket broadcast policy: prefer small patch events for high-frequency updates (e.g. token drags fire `combat:token-position-patch` with just `{tokenId, position}` instead of re-broadcasting the full combat state). Reserve full-state broadcasts (`combat:state-update`) for authoritative transitions — turn advances, round changes, GM edits, and join/resync. The client keeps the last full state cached (`_pendingPlayerCombatState` in `js/socket-client.js`) and merges patches into it before re-rendering. Late joiners always receive the full state via `combat:request-state` → `combat:state`, so patch-only updates never desync.
- DB query policy: never issue one query per item in a loop. Use `WHERE id = ANY($1::int[])` (or equivalent batched form) and reassemble results with a `Map` keyed by id. See `rebuildPool()` in `server/sockets/handlers.js` for the canonical pattern.
- Inline-style policy: HTML pages and JS-built UI must not contain static `style="..."` attributes or static `element.style.X = ...` assignments. Move static appearance into the page's CSS file (`public/css/<page>.css`) or the shared base layer in `css/input.css` and reference it via a class (or BEM-style modifier like `.tm-ctx-item--danger`, `.deal-label--package`, `.cb-status-error`, `.sum-save-status--error`). Inline `style="display:none;"` for elements whose visibility is toggled at runtime by JS is the documented exception (`element.style.display = ''` resets to the cascade, which would re-hide if the rule lived in CSS). The `<body>` background/text colors are now applied centrally via `@layer base body { ... }` in `css/input.css`, so individual pages do not need to set them inline.
...
- **ALLOWED_MAPS:** `['burning-deck', 'switch-lair', 'landing-field', 'vanishing-place', 'banshee', 'jungle-trek', 'blackwind-point', 'filtration-plant', 'gladiator-pit', 'aviary', 'knife-in-the-dark', 'command-center', 'dungeons', 'throne-room', 'throne-room-court']` in `server/index.js`. Map titles in `/api/maps/list` are derived from `MAPS_META` (no separate title map). Add new maps to both `ALLOWED_MAPS` and `MAPS_META`, and add an `<option>` + `MAPS` entry to `public/maps/editor.html`.
...
**DB tables:**
- `holonet_broadcasts` — tracks broadcast history (feed_id, story_ids, broadcast_by, broadcast_at). Acts as the persistence layer for the "already-sent" set.
- `holonet_custom_stories` (Task #205) — GM-authored custom stories created at the table without editing JSON. Columns: `id`, `story_id` (text uuid like `custom_<base36>_<rand>`), `headline`, `source`, `body`, `channel`, `story_type`, `air_date` (optional Imperial-string), `created_at`, `updated_at`. Loaded by `_loadHolonetCustomStories()` and merged into the canonical feeds as a synthetic `feed_gm_custom` ("GM Custom Stories") feed in both `/feeds` and `/queue`. Custom stories are partitioned into `ready`/`evergreen` by the same airDate logic as canonical entries.

**Server:**
- `GET /api/campaign/holonet/feeds` — returns canonical feeds (player-accessible). When the caller is a GM (cookie role check via `roleFromCookie`), the response also includes the synthetic `feed_gm_custom` feed and a top-level `customStories` array; players only see canonical feeds so unpublished GM drafts/scheduled custom stories never leak before broadcast.
- `GET /api/campaign/holonet/history` — broadcast history
- `GET /api/campaign/holonet/queue` — Task #201; partitions all not-yet-broadcast stories (canonical + GM custom) into `{ ready, evergreen }`. `ready` = stories whose `airDate` resolves to `airDayIndex <= currentDayIndex`. Returns `{ readyCount, currentDayIndex, ready, evergreen, broadcastedIds }`.
- `POST /api/campaign/holonet/broadcast` — GM broadcasts selected stories (canonical or custom), emits `holonet:incoming` socket event to players room **and** `holonet:queue-updated` (broadcast) so GM clients refresh.
- Task #205 CRUD (GM-only): `GET /api/campaign/holonet/custom-stories`, `POST /api/campaign/holonet/custom-stories`, `PATCH /api/campaign/holonet/custom-stories/:id`, `DELETE /api/campaign/holonet/custom-stories/:id`. POST/PATCH validate that `headline`/`source`/`body` are non-empty, `channel` is one of `imperial|underworld|pirate|fringe-trade`, `type` is one of `propaganda|lore|foreshadow|flavor`, and any provided `airDate` round-trips through `GalacticCalendar.parseImperialString` (otherwise 400 with a hint). All three write endpoints emit `holonet:queue-updated` so the GM tile badge refreshes.
...
**Decision Point System:** Adventures use a `decisionPoints` array on scenes for campaign-affecting choices. Each decision point has `id`, `prompt`, `campaignImpact` (registry key), and `options[]` with `key`, `label`, `sets` (impact value), and `consequence`. Old flat `decisions` arrays remain for backward compat. The `data/decision-registry.json` file defines all campaign-level impact keys (switch-deal, mandrake-fate, kessra-grudge, raden-fate, denia-fate, maya-fate, sinde-cipher, varth-relationship, malpaz-uprising, soren-alliance) with defaults and source scenes. DB table `campaign_decisions` has columns: `decision_point_id`, `option_key`, `impact_value`, `gm_notes`, `auto_notes`, `vote_data` (JSONB — persists who voted what, tally, total). **Decision Resolver:** `server/utils/decision-resolver.js` — `resolveDecisionState()` loads defaults from registry file, then applies `impact_value` from DB rows directly. Falls back to keyword matching for legacy decisions. `applyAdventureConditionals(adventure, state)` deep-clones and processes all conditionals on scenes/parts/NPCs. **Campaign State Panel:** The GM UI sidebar shows a live campaign state grid (all impact keys + current values). **Decision Modal:** Supports structured option selection from `decisionPoints`, auto-fills impact key/value, includes GM notes field.
...
### GM Black Ledger visibility & Destiny Link (Task #244)
- **Adventure goals default hidden:** all `adv1.json`–`adv10.json` mark entries except one starter per adventure now ship `hidden: true`. Crucibles/finales preserved with `noHide: true` (also rendered hidden but never auto-promoted). Each adventure exposes 1 starter + 1 noHide crucible visible by default; the rest stay sealed until the GM reveals them.
- **GM Cockpit — Goals to Reveal:** `js/advancement-panel.js` `_buildGoalsToRevealCockpit()` renders a collapsible card at the top of the Advancement Engine (GM-only). Hidden marks are grouped by Part (`part_title` / falls back to part code), each row shows label + scene + desc + a Reveal button that POSTs `/api/campaign/adventures/:advId/marks/:markId/reveal`. Existing per-row `Reveal to Players` buttons still live in the checklist and now use a `:not([data-gtr-reveal-mark])` selector so they don't double-bind. Cockpit collapse state cached in `_gtrCollapsed`. Empty state shows "All adventure goals are revealed."
- **Server marks endpoint enrichment:** `server/routes/campaign.js` `GET /campaign/adventures/:adventureId/marks` now returns `part_title` + `scene_title` per mark plus top-level `act` and `adventureTitle`. Reveal/Hide endpoints gained `requireGM`-style guard (`if (req.userRole && req.userRole !== 'gm') 403`) — defense in depth since the cockpit is the only client surface that fires them.
- **NPC connection leakage fix:** `server/routes/npc-profiles.js` adds `_filterPlayerConnections` (parses leading subject token before em/en/hyphen, case-insensitive match against revealed NPC name/key alphabet, drops lines pointing to NOT-yet-revealed NPCs, KEEPS lines whose subject doesn't match any NPC name — those are objects/places/factions). Applied at: `GET /npc-profiles/revealed`, `POST /npc-profiles/push-all`, `POST /:npcKey/reveal` (broadcasts a re-filtered roster via `_rebroadcastRevealedRoster(io)`), and the socket handlers `npc:request-sync` and `npc:push-update`. GM endpoints unchanged — full connection text retained for GM eyes only.
- **DP profile reveal reset on campaign wipe:** `server/db.js` flipped seed defaults for `maya/varth/varga` from `revealed:true` to `revealed:false`. One-shot migration `dp_profile_seed_reveal_reset_v1` (gated by `app_settings`) flips existing rows back to hidden so existing installs reset on next boot. Campaign-wipe path already wipes `npc_profiles` and reseeds, so a fresh wipe also gets the new defaults.
- **Destiny Link panel (no auto-link):** removed per-PC inline linker rows from `js/command-bridge.js` Black Ledger card grid (~L4277-4357 area). Added a floating `Destiny Link` button (`#cb-destiny-link-btn`) under Builders in `public/gm/index.html` that opens a panel listing every PC with a per-PC dropdown — partner choices are ALL other PCs in the party (no destiny-tag matching, no auto-suggest). Selection persists to `advancement.linkedPartners[currentAct]` via the existing `_saveLinkedPartner` flow. Default state for every PC is "Unlinked." `js/advancement-panel.js` adds a `Shares to: <Name>` blue badge on destiny-tagged trigger rows when (a) the trigger matches the PC's destiny road AND (b) a linked partner is set for the current Act; otherwise no badge.
- **CSS classes added in `css/input.css`:** `.adv-tag--shares` (blue badge for Shares-to), and the `.adv-gtr-cockpit/.adv-gtr-header/.adv-gtr-chevron/.adv-gtr-title/.adv-gtr-count/.adv-gtr-empty/.adv-gtr-help/.adv-gtr-groups/.adv-gtr-group/.adv-gtr-group-title/.adv-gtr-row/.adv-gtr-row-info/.adv-gtr-row-label/.adv-gtr-row-scene/.adv-gtr-row-desc/.adv-gtr-row-btn` family for the cockpit. Tailwind watcher rebuilds `public/css/output.css` automatically.
- **Render/binding pattern:** `_render()` does `container.innerHTML = html` then `_bindEvents(container)`, so the innerHTML wipe destroys old DOM + listeners — no double-bind risk for the new cockpit handlers. `_getLinkedPartnerName()` null-guards both `_advancement` and `_currentAdventureAct`.

### Validation-gate fixes (Task #198, second pass)
- **Conversation overlay live clock subscription:** wrapped the conversation date line in `_renderConvDate()` with a `.conv-p-date-mount` container; subscribes to `GalacticClock.onChange` and stores the unsubscribe on `win.__convClockUnsub`. Both the hide-button path and the `renderPlayerModal()` re-render-replace path now release the subscription before removing the node, so listeners do not accumulate across re-renders.
- **Holiday banner color coding:** added `HOLIDAY_TYPE_COLORS` map (anchor / imperial-mandatory / imperial-shadow / imperial-oppression / imperial-atrocity / suppressed / suppressed-trauma / rebel-emergence / hutt-cover / hutt-economy / festival) in `js/clock-widget.js` and a new `#cb-clock-banner` div under the GM widget in `public/gm/index.html`. `_renderGmWidget()` paints bg/border/fg from the type map (falls back to `festival` for unknown types) and also tints the upcoming-holiday hint when ≤14 days out.
- **Scene-level timeAdvance backfill:** all 14 scenes of adv1.json now carry `{ hours? days? label }` hints — intra-day combat scenes are 1–6h, hyperspace transit Jakku→Bestine is +1d, the long-way-home scene is +2d.
- **Command-bridge GM prompt flows:** `selectAdventure()` captures `prevAdventure`/`prevScene` and, on actual switch, fires `_maybePromptAdventureCompletion()` (only when the previous scene was the last scene of the last part) and then `_maybePromptStartDate()` (parses `adv.startDate` via `GalacticCalendar.parseImperialString`). `navigateScene()` (forward only) calls `_maybePromptSceneTimeAdvance()` for the leaving scene, which POSTs `/api/campaign/clock/advance` on confirm. Adventure-completion prompt opens the existing GM clock advance modal via the widget click.
- **`GalacticCalendar.parseImperialString()`:** new parser supporting "Primeday, 4 Elona, Year 4" / "4 Elona, Year 4" / "Day D, Month M, Year Y" / "Fete Week, Day N, Year Y" / "Festival Day N, Year Y". Required extending `dayIndexFromDate()` to honor `isFete/feteDay` and `isStandalone/standaloneDay` fields (previously ignored — produced wrong dayIndex for fete/festival forms). All five formats verified via Node round-trip.
