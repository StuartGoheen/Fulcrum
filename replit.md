...
- `data/threats.json` — NPC Threat Builder data: Fulcrum rules system (Tier 0-5, Arena Scores 1-5, 13 Roles, 4 Classifications, 52 Traits, 26 Tags). NPC stat formulas: Initiative=Tier+(Wits×2)+Mods, Defense=max(Physique,Reflex)+Tier, Evasion=Reflex+Tier, Resist=Grit+Tier, Vitality=(Physique+Grit+Tier)×classMod, Power=Arena+Tier. Unopposed actions (defense "none") auto-resolve at the NPC's Power tier — F(1-3), M(4-7), L(8+) — with the active tier highlighted on the card. 5 threat categories: character (scale 1), vehicle (scale 2), starship (scale 3), capital_ship (scale 4), station (scale 5). Scale traits have `autoApply: true`. Traits support `powerMod` (Turbolaser Battery, Overgunned, Mounted Weapon) and `initiativeMod` (Advanced Sensors, Command Ship). Ship-flavored arena labels: Firepower/Handling/Hull/Sensors/Command for ships; Armor/Handling/Hull/Sensors/Presence for vehicles. Ship details: hullType, crew, speed, shields, hyperdrive, sensors, cargo. GM Command Bridge NPC cards show base arenas (PHY/REF/GRT/WIT/PRE) with +/- modifier buttons for conditions (Weakened/Empowered); derived stats auto-recalculate live
