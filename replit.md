...
- **ALLOWED_MAPS:** `['burning-deck', 'switch-lair', 'landing-field', 'vanishing-place', 'banshee', 'jungle-trek', 'blackwind-point', 'filtration-plant', 'gladiator-pit', 'aviary', 'knife-in-the-dark']` in `server/index.js`. Map titles in `/api/maps/list` are derived from `MAPS_META` (no separate title map). Add new maps to both `ALLOWED_MAPS` and `MAPS_META`, and add an `<option>` + `MAPS` entry to `public/maps/editor.html`.
